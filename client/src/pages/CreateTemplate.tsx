  ...
  return (
    <>
      <div className="flex flex-col gap-3">
        {/* ✅ FIX: Always visible Expand Button above the table */}
        <div className="flex justify-end px-1">
          <button 
            onClick={() => setIsModalOpen(true)} 
            className="flex items-center gap-1.5 text-xs font-bold text-[#059669] bg-[#ECFDF5] border border-[#A7F3D0] hover:bg-[#D1FAE5] px-3 py-1.5 rounded-lg transition-colors shadow-sm"
          >
            <Maximize2 size={14} /> Expand Table View
          </button>
        </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#111827]/60 backdrop-blur-sm p-4 md:p-8 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-7xl max-h-full flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#E5E7EB] bg-white z-10">
              <div>
                <h3 className="text-xl font-bold text-[#111827] flex items-center gap-2"><FileSpreadsheet className="text-[#059669]" size={24}/> Expanded Data Preview</h3>
                <p className="text-sm text-[#6B7280] mt-1">Showing {rows.length} preview rows and {schema?.length || 0} columns.</p>
              </div>
              <button onClick={() => setIsModalOpen(false)} className="p-2 text-[#9CA3AF] hover:text-[#111827] bg-[#F3F4F6] hover:bg-[#E5E7EB] rounded-lg transition-colors"><X size={20} /></button>
            </div>
            <div className="flex-1 overflow-auto bg-[#F8FAFC] p-6">
...
export default function TemplateWizard() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setMatchedCount(res.row_count || 0);
      setDatasetMeta({ name: file.name, count: res.row_count || 0 });
      setConditions([]); // Reset filter on new upload
  ...
  // Helper for dynamic operators
  const getOperatorsForType = (type?: string) => {
    if (type === 'number') return ['==', '<', '>', '<=', '>='];
    return ['==', 'contains'];
  };

  // --- STEP 3: MESSAGE COMPOSER LOGIC ---
  const insertVariablePill = (varName: string) => {
    if (!editorRef.current) return;
    if (document.activeElement !== editorRef.current) {
      editorRef.current.focus();
  ...
  const handleCreateTemplate = async () => {
    ...
    if (missingVariables.length > 0) {
      showToast(`Invalid variable(s): ${missingVariables.join(', ')}. Use variables from schema.`, "error");
      setActiveStep(3); 
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans text-[#111827] py-10 px-6">
      ...
      {/* HEADER & TOP STEPPER (Widened to 1200px) */}
      <div className="max-w-[1200px] mx-auto mb-10 flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#111827]">Create Template</h1>
        ...
        {cachedState && (
          <button onClick={clearDraft} className="flex items-center gap-1.5 text-xs font-bold text-[#6B7280] hover:text-red-600 transition-colors bg-white px-3 py-1.5 rounded-md shadow-sm border border-[#E5E7EB]">
            <RefreshCw size={14}/> Reset Flow
          </button>
        )}
      </div>

      <div className="mt-8 flex items-center justify-between w-full max-w-4xl mx-auto relative mb-12">
        <div className="absolute left-0 top-1/2 -translate-y-1/2 w-full h-1 bg-[#E5E7EB] z-0 rounded-full" />
        <div className="absolute left-0 top-1/2 -translate-y-1/2 h-1 bg-[#059669] z-0 rounded-full transition-all duration-300" style={{ width: `${((activeStep - 1) / 3) * 100}%` }} />
      ...
      {/* MAIN CONTENT CARD (Widened to 1200px) */}
      <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-sm max-w-[1200px] mx-auto overflow-hidden flex flex-col min-h-[500px]">
        <div className="p-8 flex-1">
          ...
          {activeStep === 1 && (
            <div className="animate-in fade-in zoom-in-95 duration-300 space-y-8">
              {!tempDatasetId ? (
                <div className="border-2 border-dashed border-[#E5E7EB] rounded-2xl bg-[#F8FAFC] p-24 flex flex-col items-center justify-center hover:border-[#059669] hover:bg-[#059669]/5 transition-colors group relative cursor-pointer max-w-2xl mx-auto mt-8">
                  <input type="file" accept=".csv" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" onChange={handleFileUpload} />
                  <UploadCloud className="text-[#059669] mb-4 group-hover:scale-110 transition-transform" size={48} />
                  <h3 className="text-lg font-bold text-[#111827]">Drag & Drop CSV here</h3>
                  <p className="text-sm text-[#6B7280] mt-2">This temporary dataset will be linked to your new template.</p>
                </div>
              ) : (
                <div className="space-y-6">
                  {/* Summary & Replace */}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    {/* Data Preview Column */}
                    <div className="md:col-span-3 bg-white border border-[#E5E7EB] rounded-xl overflow-hidden flex flex-col shadow-sm">
                      <div className="bg-[#F8FAFC] border-b border-[#E5E7EB] px-4 py-3 flex items-center gap-2">
                         <Eye size={16} className="text-[#059669]" />
          ...
          {activeStep === 2 && (
            <div className="animate-in fade-in zoom-in-95 duration-300 space-y-6">
              <div>
                <h2 className="text-xl font-bold text-[#111827]">Define Target Audience (Optional)</h2>
                <p className="text-sm text-[#6B7280] mt-1">If no filter is set, the template will apply to all {datasetMeta?.count} rows by default.</p>
              </div>

              {/* Strict Filter Examples Banner */}
              <div className="bg-[#F8FAFC] border border-[#E5E7EB] rounded-xl p-4 flex gap-4 text-sm mb-6 shadow-sm">
                <Info className="text-[#059669] shrink-0 mt-0.5" size={18} />
                <div className="w-full">
                  <p className="font-semibold text-[#111827] mb-2">Filter Examples</p>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-white border border-[#E5E7EB] p-3 rounded-lg shadow-sm"><span className="text-xs text-[#6B7280] font-bold uppercase tracking-wider block mb-1">Example 1 (Number)</span> <span className="font-mono text-[#059669] font-bold text-sm">attendance &lt; 75</span></div>
                    <div className="bg-white border border-[#E5E7EB] p-3 rounded-lg shadow-sm"><span className="text-xs text-[#6B7280] font-bold uppercase tracking-wider block mb-1">Example 2 (String)</span> <span className="font-mono text-blue-600 font-bold text-sm">rollno == 23951A</span></div>
                    <div className="bg-white border border-[#E5E7EB] p-3 rounded-lg shadow-sm"><span className="text-xs text-[#6B7280] font-bold uppercase tracking-wider block mb-1">Example 3 (Contains)</span> <span className="font-mono text-blue-600 font-bold text-sm">name contains john</span></div>
                  </div>
                </div>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                {/* BUILDER (Left side - wider) */}
                <div className="lg:col-span-5 bg-white border border-[#E5E7EB] rounded-2xl shadow-sm p-6 h-max">
                  <div className="mb-6 bg-[#F8FAFC] border border-[#E5E7EB] p-4 rounded-xl">
                    <p className="text-sm font-semibold text-[#111827] mb-3">Match Conditions:</p>
                    <div className="flex gap-6">
                      <label className="flex items-center gap-2 text-sm text-[#4B5563] cursor-pointer">
                  ...
                  <div className="space-y-4">
                    {conditions.length === 0 && (
                      <p className="text-sm text-[#059669] bg-[#ECFDF5] p-4 rounded-lg border border-[#A7F3D0] flex items-center gap-2">
                        <CheckCircle size={16}/> No filter applied.
                      </p>
                    )}
                    {conditions.map((cond, idx) => {
                      ...
                      return (
                        <div key={cond.id} className="flex items-center gap-2 bg-[#F8FAFC] p-2 rounded-xl border border-[#E5E7EB]">
                          <select value={cond.column} onChange={e => { const newConds = [...conditions]; newConds[idx].column = e.target.value; newConds[idx].operator = '=='; setConditions(newConds); setFilterTested(false); }} className="w-[35%] px-3 py-2 bg-white border border-[#E5E7EB] rounded-lg text-sm focus:border-[#059669] outline-none shadow-sm">
                            <option value="" disabled>Column</option>
                            {schema?.map(col => <option key={col.name} value={col.name}>{col.name} ({col.type})</option>)}
                          </select>
                          <select value={cond.operator} onChange={e => { const newConds = [...conditions]; newConds[idx].operator = e.target.value; setConditions(newConds); setFilterTested(false); }} className="w-[30%] px-3 py-2 bg-white border border-[#E5E7EB] rounded-lg text-sm focus:border-[#059669] outline-none shadow-sm font-mono">
                            {validOperators.map(op => <option key={op} value={op}>{op}</option>)}
                          </select>
                          <input type={isNum ? "number" : "text"} placeholder="Value" value={cond.value} onChange={e => { const newConds = [...conditions]; newConds[idx].value = e.target.value; setConditions(newConds); setFilterTested(false); }} className="w-[35%] px-3 py-2 bg-white border border-[#E5E7EB] rounded-lg text-sm focus:border-[#059669] outline-none shadow-sm" />
                          <button onClick={() => {
                            const newConds = conditions.filter(c => c.id !== cond.id);
                            setConditions(newConds);
                            if (newConds.length === 0) setFilterTested(true); // Auto-pass if empty
                          }} className="text-[#9CA3AF] hover:text-red-600 bg-white p-2 rounded-lg border border-[#E5E7EB] hover:border-red-200 transition-colors shadow-sm"><Trash2 size={16} /></button>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  <div className="mt-6 flex items-center justify-between pt-4 border-t border-[#E5E7EB]">
                    <button onClick={() => setConditions([...conditions, { id: Date.now().toString(), column: schema[0]?.name || '', operator: '==', value: '' }])} className="text-sm font-bold text-[#111827] bg-white border border-[#E5E7EB] px-4 py-2 rounded-lg shadow-sm flex items-center gap-1.5 hover:bg-[#F9FAFB] transition-colors"><Plus size={14} /> Add Condition</button>
                    {conditions.length > 0 && (
                  ...
                  {filterTested && conditions.length > 0 ? (
                    <div className="flex flex-col h-full animate-in fade-in bg-white border border-[#E5E7EB] rounded-2xl p-6 shadow-sm">
                      <div className={`p-4 rounded-xl border flex items-center gap-3 mb-4 ${matchedCount === 0 ? 'bg-red-50 text-red-700 border-red-200' : 'bg-[#ECFDF5] text-[#047857] border-[#A7F3D0]'}`}>
                        <Users size={20} />
                        <span className="text-base font-bold">{matchedCount} Records Matched</span>
                      </div>
                      <DataTable rows={matchedRows} schema={schema} emptyMessage="⚠ No rows matched your filter." />
                    </div>
                  ) : (
                     <div className="flex-1 border-2 border-dashed border-[#E5E7EB] rounded-2xl bg-[#F8FAFC] flex flex-col items-center justify-center p-12 text-center">
                        <Filter className="text-[#9CA3AF] mb-4" size={48} />
                        ...
                        {csvFile === null && conditions.length > 0 && <p className="text-sm text-red-600 font-bold mt-3">CSV file lost from cache. Please re-upload on step 1 to test filter.</p>}
                     </div>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="space-y-5">
                  <div className="bg-white border border-[#E5E7EB] p-6 rounded-2xl shadow-sm">
                    ...
                    <input type="text" placeholder="e.g., Low Attendance Alert" value={templateName} onChange={e => setTemplateName(e.target.value)} className="w-full px-4 py-3 bg-[#F8FAFC] border border-[#E5E7EB] rounded-lg text-sm font-medium outline-none focus:bg-white focus:border-[#059669] transition-colors mb-5" />
                  ...
                  <div>
                    <div className="flex gap-2 flex-wrap bg-[#F8FAFC] p-5 rounded-t-2xl border border-[#E5E7EB] border-b-0 shadow-sm">
                      <span className="block w-full text-xs font-bold text-[#9CA3AF] uppercase tracking-wider mb-2">Insert Schema Variables</span>
                      {schema?.map(col => (
                    ...
                    <div 
                      ...
                      onInput={(e) => setEditorHtml(e.currentTarget.innerHTML)}
                      onKeyDown={handleEditorKeyDown}
                      className="w-full h-64 p-6 border border-[#E5E7EB] rounded-b-2xl text-base outline-none focus:border-[#059669] shadow-inner bg-white overflow-y-auto leading-relaxed text-[#111827]"
                      style={{ whiteSpace: 'pre-wrap' }}
                      suppressContentEditableWarning={true}
                    />
                  </div>
                ...
                <div className="bg-[#F8FAFC] border border-[#E5E7EB] rounded-2xl p-8 flex flex-col shadow-sm h-full">
                  <h3 className="text-sm font-bold text-[#111827] flex items-center gap-2 mb-6 uppercase tracking-wider border-b border-[#E5E7EB] pb-3">
                    <Eye size={18} className="text-[#059669]"/> Live Rendering Preview
                  </h3>
                  <div 
                    className="bg-white border border-[#E5E7EB] rounded-xl p-8 flex-1 shadow-sm text-base text-[#111827] whitespace-pre-wrap leading-relaxed" 
                    dangerouslySetInnerHTML={{ __html: getLivePreview() || '<span class="text-gray-400 italic">Start typing on the left to see your preview here...</span>' }} 
                  />
                  <p className="text-[10px] text-[#9CA3AF] mt-4 uppercase font-bold tracking-wider text-center">Using Row 1 Data</p>
                </div>
              </div>
            </div>
          )}

          {/* 4️⃣ STEP 4: REVIEW & CREATE */}
          {activeStep === 4 && (
            <div className="animate-in fade-in zoom-in-95 duration-300 space-y-8">
              <div className="text-center max-w-lg mx-auto mb-10 mt-8">
                <div className="w-20 h-20 bg-[#ECFDF5] rounded-full flex items-center justify-center mx-auto mb-6 border border-[#A7F3D0] shadow-sm">
                  <Send className="text-[#059669]" size={40} />
                </div>
                <h2 className="text-3xl font-bold text-[#111827]">Review & Create Template</h2>
                <p className="text-base text-[#6B7280] mt-3">Verify your configuration. Creating this template will make it available for campaigns instantly.</p>
              </div>

              <div className="bg-[#F8FAFC] border border-[#E5E7EB] rounded-2xl p-10 max-w-4xl mx-auto shadow-sm">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10 divide-y md:divide-y-0 md:divide-x divide-[#E5E7EB]">
                  <div className="space-y-8">
                    <div>
                      <p className="text-xs text-[#6B7280] uppercase tracking-wider font-bold mb-2">Template Name</p>
                      <p className="text-lg font-bold text-[#111827]">{templateName}</p>
                    </div>
                    <div>
                      <p className="text-xs text-[#6B7280] uppercase tracking-wider font-bold mb-2">Dataset Linked</p>
                      <p className="text-base font-medium text-[#111827] flex items-center gap-2 bg-white px-3 py-2 rounded-lg border border-[#E5E7EB] w-max"><FileSpreadsheet size={18} className="text-[#059669]"/> {datasetMeta?.name || 'None'}</p>
                    </div>
                  </div>
                  <div className="pt-8 md:pt-0 md:pl-10 space-y-8 flex flex-col justify-center">
                    <div>
                      <p className="text-xs text-[#6B7280] uppercase tracking-wider font-bold mb-2">Target Audience Size</p>
                      <p className="text-4xl font-black text-[#059669] flex items-baseline gap-2">
                        {matchedCount ?? datasetMeta?.count ?? 0} <span className="text-sm font-bold text-[#6B7280] uppercase tracking-wider">Records</span>
                      </p>
                    </div>
                    {conditions.length === 0 && (
                      <div className="bg-blue-50 text-blue-800 text-sm px-4 py-2.5 rounded-lg font-bold border border-blue-200 inline-block w-max">
        ...
        {/* BOTTOM ACTION BAR */}
        <div className="px-10 py-6 border-t border-[#E5E7EB] bg-[#F8FAFC] flex items-center justify-between">
          {activeStep > 1 && (
            <button onClick={prevStep} className="mr-auto px-6 py-3 flex items-center gap-2 text-sm font-bold text-[#4B5563] bg-white border border-[#E5E7EB] rounded-xl hover:text-[#111827] hover:bg-[#F9FAFB] shadow-sm transition-colors">
              <ChevronLeft size={16} /> Back
            </button>
          )}
          
          {activeStep < 4 ? (
            <button onClick={nextStep} disabled={!canProceed()} className="px-10 py-3.5 bg-[#111827] text-white text-base font-bold rounded-xl hover:bg-[#1F2937] shadow-lg flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all ml-auto">
              Continue <ChevronRight size={18} />
            </button>
          ) : (
             <button onClick={handleCreateTemplate} disabled={isCreating} className="px-12 py-4 bg-[#059669] text-white text-lg font-bold rounded-xl hover:bg-[#047857] shadow-xl shadow-[#059669]/20 flex items-center gap-3 transition-all disabled:opacity-50 ml-auto">
              {isCreating ? 'Creating Template...' : 'Create Template'} <Send size={22} />
            </button>
          )}
        </div>
